# Zed-Thon - ZelZal
# Copyright (C) 2022 ZZZ7iZ . All Rights Reserved
#
...
ANTI_DDDD_ZZZ7iZ_MODE = ChatBannedRights(
...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس سبارك ",
        "الاوامـر": {
    ...
    if is_locked(zed_id, "game") and event.message.media:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in zed_dev:
	        ...
	        try:
	            ...
	            await event.client(
	                EditBannedRequest(
	                    event.chat_id, event.sender_id, ANTI_DDDD_ZZZ7iZ_MODE
	                )
	            )
	        except Exception as e:
	            await event.reply(
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - zthon" "زلـزال الهيبـه"
    zed_dev = (5012406813, 5012406813, 5012406813)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙎𝙋𝘼𝙍𝙆\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝙎𝙋𝘼𝙍𝙆\
