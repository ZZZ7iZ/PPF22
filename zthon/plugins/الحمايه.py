...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆  **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5012406813 or event.chat_id == 5012406813 or event.chat_id == 5012406813 or event.chat_id == 5012406813:
        reason = "**انـه مطـور السـورس **"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
