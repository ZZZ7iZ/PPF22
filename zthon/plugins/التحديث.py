...
async def update_bot(event, repo, ups_rem, ac_br):
    ...
    sandy = await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**•⎆┊تم التحـديث ⎌ بنجـاح**\n**•⎆┊جـارِ إعـادة تشغيـل بـوت سـبـارك ⎋ **\n**•⎆┊انتظـࢪ مـن 2 - 1 دقيقـه . . .📟**")
    await event.client.reload(sandy)


async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is None:
        return await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n **•─────────────────•**\n** ⪼ لم تقـم بوضـع مربـع فـار HEROKU_API_KEY اثنـاء التنصيب وهـذا خطـأ .. قم بضبـط المتغيـر أولاً لتحديث بوت سـبـارك ..؟!**", link_preview=False)
    heroku = heroku3.from_key(HEROKU_API_KEY)
    heroku_applications = heroku.apps()
    if HEROKU_APP_NAME is None:
        await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n **•─────────────────•**\n** ⪼ لم تقـم بوضـع مربـع فـار HEROKU_APP_NAME اثنـاء التنصيب وهـذا خطـأ .. قم بضبـط المتغيـر أولاً لتحديث بوت سـبـارك ..؟!**", link_preview=False)
        repo.__del__()
        return
    heroku_app = next(
    ...
    if heroku_app is None:
        await event.edit(
            f"{txt}\n" "**- بيانات اعتماد هيروكو غير صالحة لتنصيب تحديث سـبـارك**"
        )
        return repo.__del__()
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n**⪼ جاري البحث عن التحديثات  🌐.. ،**")
    off_repo = UPSTREAM_REPO_URL
    force_update = False
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n** ⪼ اضبط الفـارات المطلوبة أولاً لتحديث بوت سـبـارك ،**"
        )
    try:
        txt = (
    ...
    if changelog == "" and not force_update:
        await event.edit(
            f"\nᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**⪼ سـورس سـبـارك محـدث لـ آخـر إصـدار 🛂**"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        return await edit_or_reply(event, f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**•⎆┊يوجـد تحـديث جديـد لسـورس سـبـارك ༗...**\n\n**•⎆┊للتحديث السريع اضغـط هنـا ⇜** ⦉ `{cmdhd}تحديث الان` ⦊ \n**•⎆┊للتحديث الجـذري اضغـط هنـا ⇜** ⦉ `{cmdhd}تحديث البوت` ⦊ \n\n𓆩 [𝙎𝙊𝙐𝙍𝘾 𝙎𝙋𝘼𝙍𝙆](t.me/ZZZ7iZ) 𓆪")
    if force_update:
        await event.edit(
            "`Force-Syncing to latest stable userbot code, please wait...`"
        )
    if conf == "الان":
        await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت سـبـارك .. انتظـر . . .🌐**")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت سـبـارك .. انتظـر . . .🌐**\n\n%𝟷𝟶 ▬▭▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت سـبـارك .. انتظـر . . .🌐**\n\n%𝟸𝟶 ▬▬▭▭▭▭▭▭▭▭")
        await asyncio.sleep(1)
        await event.edit("ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**⇜ يتـم تحـديث بـوت سـبـارك .. انتظـر . . .🌐**\n\n%𝟹𝟶 ▬▬▬▭▭▭▭▭▭▭")
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "" and not force_update:
            ...
            return await event.respond(
                f"⌔ :  لتحديث سورس سبارك ارسل : `.تحديث الان` "
            )
    ...
    if event.reply_to and event.sender_id in progs:
        ...
        if owner_id == zedub.uid:
            if event.message.message == "حدث":
                conf = "الان"
                event = await event.reply("**ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙎𝙋𝘼𝙍𝙆   - تحـديثـات السـورس\n**•─────────────────•**\n\n**✾╎جـاري تـحــديــث الـسـورس بأمــر مـن المـطور ⎌****")
                off_repo = UPSTREAM_REPO_URL
                force_update = False
